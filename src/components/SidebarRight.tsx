...
const SidebarRight: React.FC<SidebarRightProps> = ({ selectedIds, blocks, onUpdate, onAlign, onPlaceInRegion, onChangeZIndex }) => {
  ...
  const zIndexControls = (
    <Row label="重なり順">
      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
        <button className="btn" style={{ flex: 1, justifyContent: 'center' }} onClick={() => onChangeZIndex(selectedIds, 'front')} title="最前面へ">最前面</button>
        <button className="btn" style={{ flex: 1, justifyContent: 'center' }} onClick={() => onChangeZIndex(selectedIds, 'forward')} title="前面へ">前面</button>
        <button className="btn" style={{ flex: 1, justifyContent: 'center' }} onClick={() => onChangeZIndex(selectedIds, 'backward')} title="背面へ">背面</button>
        <button className="btn" style={{ flex: 1, justifyContent: 'center' }} onClick={() => onChangeZIndex(selectedIds, 'back')} title="最背面へ">最背面</button>
      </div>
    </Row>
  );
  ...
  const fontRow = hasFontRow && (
    <Row label="フォント">
      <select className="select-input" value={(block as any).fontFamily || '"UD Digi Kyokasho N-R", "UD デジタル 教科書体 N-R", "Yu Mincho", "MS Mincho", serif'}
  ...
  const writingModeRow = 'writingMode' in block && (
    <Row label="文字の向き">
      <select className="select-input" value={(block as any).writingMode}
  ...
  const fontSizeRow = 'fontSize' in block && (
    <Row label="文字サイズ">
      <input type="number" min={8} max={120} className="select-input"
  ...
  const sizeRow = (
    <>
      <Row label="幅 (px)">
        <input type="number" min={10} className="select-input" value={block.width}
          onChange={e => onUpdate(block.id, { width: parseInt(e.target.value) || block.width })}
        />
      </Row>
      <Row label="高さ (px)">
        <input type="number" min={10} className="select-input" value={block.height}
  ...
  const colLabels = ['左', '中央', '右'] as const;
  const placementGrid = (
  ...
  const alignRow = (
    <Row label="用紙基準で整列">
      <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
        <button className="btn" onClick={() => onAlign(block.id, 'left')} title="左揃え"><AlignLeft size={14} /></button>
        <button className="btn" onClick={() => onAlign(block.id, 'center-h')} title="水平中央"><AlignCenter size={14} /></button>
        <button className="btn" onClick={() => onAlign(block.id, 'right')} title="右揃え"><AlignRight size={14} /></button>
        <button className="btn" onClick={() => onAlign(block.id, 'top')} title="上揃え"><AlignStartVertical size={14} /></button>
        <button className="btn" onClick={() => onAlign(block.id, 'center-v')} title="垂直中央"><AlignCenterVertical size={14} /></button>
        <button className="btn" onClick={() => onAlign(block.id, 'bottom')} title="下揃え"><AlignEndVertical size={14} /></button>
      </div>
    </Row>
  );
            ...
            <Row label="行数">
              <input type="number" min={1} max={20} className="select-input" value={block.rows}
                onChange={e => {
                  ...
                  onUpdate(block.id, { rows, cellTexts: newCellTexts });
                }}
              />
            </Row>
            <Row label="列数">
